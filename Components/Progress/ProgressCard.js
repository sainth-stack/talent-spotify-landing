import React from "react";
import { progressData } from "../../utilities/progressData";

const borderColorClasses = {
  "border-violet-500": "border-primary",
  "border-blue-500": "border-info",
  "border-green-500": "border-success",
  "border-orange-500": "border-warning",
};

export const ProgressCircle = ({
  percentage,
  borderColor = "border-danger",
}) => {
  const borderColorHex = {
    "border-violet-500": "#7C3AED",
    "border-blue-500": "#3B82F6",
    "border-green-500": "#10B981",
    "border-orange-500": "#F97316",
    "border-danger": "#DC2626",
  };

  const borderColorValue = borderColorHex[borderColor] || "#DC2626";

  const progressStyle = {
    background: `conic-gradient(${borderColorValue} ${
      percentage * 3.6
    }deg, #e6e6e6 0deg)`, 
    width: "6.3rem",
    height: "6.3rem",

    borderRadius: "50%",
    position: "absolute",
    zIndex: 1,
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    lineHeight: "8rem",
  };

  return (
    <div
      className="position-relative  d-flex align-items-center justify-content-center"
      style={{
        width: "6rem",
        height: "6rem",
      }}
    >

      <div style={progressStyle}></div>

      <div
        className="rounded-circle bg-white shadow d-flex align-items-center justify-content-center"
        style={{
          width: "5.5rem",
          height: "5.5rem",
          
          zIndex: 2, 
        }}
      >
        <span className="text-dark fw-bold">{percentage}%</span>
      </div>
    </div>
  );
};

const ProgressCard = () => {
  return (
    <div className="d-flex flex-column p-1 container">
      {progressData.map((data, index) => (
        <div
          key={index}
          className={`d-flex flex-column gap-2 flex-md-row my-3 justify-content-center align-items-center ${
            data.align === "right"
              ? "flex-column-reverse flex-md-row"
              : "flex-md-row"
          }`}
        >
          {data.align === "right" ? (
            <>
              <div
                className={`fs-6 my-1 fw-medium p-2 border border-2 border-dotted ${
                  borderColorClasses[data.borderColor]
                } rounded-pill me-md-4 mb-3 mb-md-0`}
              >
                {data.text}
              </div>
              <div
                className={`w-25 border-top border-dotted ${
                  borderColorClasses[data.borderColor]
                } mx-md-4`}
                style={{ width: "1.5rem" }}
              ></div>
              <ProgressCircle
                percentage={data.percentage}
                borderColor={data.borderColor}
              />
            </>
          ) : (
            <>
              <ProgressCircle
                percentage={data.percentage}
                borderColor={data.borderColor}
              />
              <div
                className={`w-25 border-top border-dotted ${
                  borderColorClasses[data.borderColor]
                } mx-md-4`}
                style={{ width: "1.5rem" }}
              ></div>
              <div
                className={`fs-6 mx-3 fw-medium p-2 border border-2 ${
                  borderColorClasses[data.borderColor]
                } rounded-pill ms-md-4 mb-3 mb-md-0`}
              >
                {data.text}
              </div>
            </>
          )}
        </div>
      ))}

      <div className="d-flex justify-content-center my-4">
        <button
          className="px-4 py-2 btn btn-primary rounded-pill"
          style={{ backgroundColor: "#083c61", borderColor: "#083c61" }}
        >
          Book A Demo
        </button>
      </div>
    </div>
  );
};

export default ProgressCard;
